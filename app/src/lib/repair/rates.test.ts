...
describe("ตัวอ่านตารางทั้งสามชุด", () => {
  ...
  it("★ ไฟล์ Excel จริงมีชื่อรายงานอยู่เหนือหัวตาราง — ต้องหาหัวตารางเจอเอง", () => {
    // เรียงแบบ "ตัวอย่าง ค่าซ่อม 1" ทุกบรรทัด: ชื่อรายงาน → ช่วงวันที่ → สาขา → แถวว่าง → หัวตาราง
    const row = (veh: string, detail: string, account: string, amount: string, fleet = "รถบริษัท") =>
      ["1", "เชียงใหม่", "1001", "ML67019597", "01/01/2567", "01/01/2567", "31/01/2567",
       fleet, veh, "ชม.70-9597", "อู่", detail, "1", "0", account, amount, "ยานยนต์"];

    const t = parseMaintenance([
      ["รายงานค่าซ่อมตามงวด"],
      ["วันที่ตามงวด ตั้งแต่ 01/01/2567 ถึง 31/01/2567"],   // มีคำว่า "วันที่ตามงวด" ปน ห้ามจับเป็นหัวตาราง
      ["สาขา : ไม่ระบุ"],
      [],
      REPORT_HEAD,
      row("รถ 12 ล้อตู้เย็น", "ค่ายาง 295/80R22.5 XMULTIZ2", "ค่ายางรถใหญ่จ่ายล่วงหน้า", "22,684.00"),
      row("หางเทรเลอร์", "ค่าเบี้ยประกันภัย พ.ร.บ. คุ้มครอง เดือน ธ.ค.66-67", "ค่าเบี้ยประกันภัยรถใหญ่", "2,718.87"),
      row("รถเทรเล่อร์ (แม่)", "เปลี่ยนนวาล์วลม", "สินทรัพย์รอตัดบัญชี", "67,582.43"),
      row("รถ 10 ล้อ", "ค่าบริการ GPS เดือน มกราคม 2567", "ค่าอุปกรณ์ติดรถอื่นๆ", "256.80", "รถร่วม"),
    ]);

    expect(t.missing).toEqual([]);
    expect(t.rows).toHaveLength(4);
    expect(t.rows.map((r) => r.amount)).toEqual([22684, 2718.87, 67582.43, 256.8]);
    expect(t.rows.every((r) => r.year === 2567)).toBe(true);

    // แบ่งประเภทตรงตามเอกสาร
    const kinds = t.rows.map((r) => classifyRow(r));
    expect(kinds.map((k) => k.driver)).toEqual(["ระยะทาง", "ระยะเวลา", "ระยะเวลา", "ระยะเวลา"]);
    expect(kinds[2]!.amount).toBeCloseTo(67582.43 / AMORTIZE_DIVISOR, 6);
  });

  it("ตารางการปฏิบัติงานที่มีหัวรายงานอยู่ด้านบนก็หาหัวตารางเจอ", () => {
    const t = parseOperations([
      ["ข้อมูลการปฏิบัติงาน ปี 2567"],
      [],
      ["ปี", "ชนิดรถ", "ประเภทรถ", "ระยะทางรวม", "จำนวนวันรวม"],
      ["2567", "รถ 6 ล้อใหญ่", "รถบริษัท", "12,000", "300"],
    ]);

    expect(t.missing).toEqual([]);
    expect(t.rows).toEqual([{ year: 2567, vehicle: "รถ 6 ล้อใหญ่", fleet: "รถบริษัท", km: 12000, days: 300 }]);
  });

...
describe("จุดระหว่างทางตั้งต้น", () => {
  ...
  it("ตัดจุดที่ต้นทางนั้นไปไม่ได้ออก — เชียงรายไม่มีเส้นไปกำแพงเพชร/ตาก/ลำพูน/เชียงดาว", () => {
    // ราชบุรี → เชียงดาว เพิ่มเข้าตารางระยะทางพร้อม 68 คู่ (24 ก.ย. 2569) จึงอยู่ครบทุกจุดแล้ว
    expect(defaultStops("ราชบุรี", "ฝาง")).toEqual(["กำแพงเพชร", "ตาก", "ลำปาง", "ลำพูน", "เชียงใหม่", "เชียงดาว"]);
    expect(defaultStops("เชียงราย", "ฝาง")).toEqual(["ลำปาง", "เชียงใหม่"]);

...
2. เปิดแดชบอร์ดรายได้ค้างไว้ — จะขึ้น "⏳ กำลังแปลงไฟล์…" (ไฟล์ละราว 10-20 วินาที)
   พอเสร็จ**หน้ารีเฟรชเอง** แถบแดง "ข้อมูลตัวอย่าง" หายไป ไม่ต้องกดอะไร
...
| `custmap.bin` หนัก 17.9 MB | commit ไว้ใน repo แล้ว (รหัสที่ hash ไว้ไม่ใช่ความลับ) โหลดทั้งก้อนเฉพาะตอนเข้าหน้าค้นรหัส หรือตอนมีรหัสต้นฉบับ hex 64 ตัวอยู่ในใบ · หน้าบันทึกข้อมูลขอแค่จำนวนระเบียนผ่าน HEAD · สร้างใหม่ด้วย `python etl/build_custmap.py "แปลงรหัสลูกหนี้รวม.xlsx"` |
| รหัสลูกค้าใหม่แยกรายเครื่อง | `custNewCodes` อยู่ใน localStorage และไม่ได้ส่งขึ้นชีต ถ้าฝ่ายบริการลูกค้าสองคนกรอกคนละเครื่อง ทั้งคู่จะเห็นเลขถัดไปตัวเดียวกันแล้วยกให้ลูกค้าคนละราย — แก้ถาวรต้องย้ายการแจกเลขไปฝั่ง Apps Script |

...
**ชื่อชนิดรถจากไฟล์ต้องผ่าน `canonVehicle()` ก่อนเสมอ** รายงานจริงพิมพ์ `รถเทรเล่อร์(แม่)` บ้าง `รถเทรเล่อร์ (แม่)` บ้าง ถ้าไม่ปรับให้ตรงคีย์ในตาราง อัตราจะถูกเก็บใต้ชื่อที่ไม่มีรถคันไหนเรียกใช้ · ปีให้อ่านจาก **วันที่ตามงวด** ก่อน `วันที่ซ่อม` เพราะรายงานจริงมีปีในวันที่ซ่อมพิมพ์ผิด (พบ `09/03/2556` ปนอยู่) · ไฟล์ `.csv` จาก Excel ไทยเป็น **Windows-874 ไม่ใช่ UTF-8** อ่านผ่าน `decodeThai()` เท่านั้น ไม่งั้นภาษาไทยกลายเป็นตัวขยะทั้งไฟล์โดยไม่มี error · ไม่รับ `.xlsx` โดยตรงเพื่อไม่ให้ต้องลากไลบรารีอ่าน Excel เข้ามาใน bundle

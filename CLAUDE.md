...
ทะเบียนรถในกองรถ: ไฟล์ต้นทางเป็นบันทึกปล่อยรถรายเที่ยว (81k แถว) ยุบเหลือ 1 ระเบียนต่อคันใน `refdata/fleet.json` — `start` = วันปล่อยรถครั้งแรก (วิ่งครั้งแรกใน ม.ค. 2567 → 1 ม.ค. 2567) · วันที่ว่างเติมจากแถวบน**ตามลำดับแถวในไฟล์** · `kinds[]` = ทุกคู่ (ประเภทรถ, ชนิดรถ) ที่คันนั้นเคยวิ่ง ฟอร์มใช้กรองทะเบียนด้วย `matchesKind()` ส่วน `fleetType/vehicle` หลักคือคู่ที่วิ่งบ่อยสุด · ชนิดรถในไฟล์ที่แอปไม่มีถูกเพิ่มใน `vehicles.json` พร้อม `approxFrom` (ยืมอัตราน้ำมัน/ความจุ) `repairKey` ใช้ชื่อเดิมเพราะ `repair.json` มีคีย์นั้นอยู่แล้ว · `lib/store/roster.ts` = ฐานจาก fleet.json ทับด้วย localStorage (`fleetRoster` เก็บเฉพาะส่วนต่าง, `fleetRoster.removed` เก็บที่ลบ)
...
ลูกค้าที่ไม่มีในไฟล์ได้รหัสรันต่อจากตัวสุดท้าย เก็บใน localStorage `custNewCodes` (`lib/custmap/newCodes.ts`) **ออกรหัสจริงตอนกดบันทึกเท่านั้น** ตอน render เป็นแค่พรีวิว ไม่งั้นเปิดหน้าดูใบเฉย ๆ ก็กินเลขไปเรื่อย ๆ · ต้อง `await ensureCustMap()` ก่อนออกรหัสเสมอ ถ้าไม่รู้ว่าไฟล์มีถึงเลขไหนจะออกเลขทับของเดิม (`registerBills` กันไว้อีกชั้นด้วยการไม่ออกรหัสเลยถ้ายังไม่มีตาราง)

...
# ชื่อเดือนภาษาไทยใช้ตัวเดียวกับ cleaning.month_label
# (ของเดิมนิยามซ้ำทั้งใน app.py:91 และ insights.py:37 — ยุบเหลือที่เดียว)
from .cleaning import month_label as _month_label  # noqa: E402
...
def quality_insights(dq: dict) -> list[dict]:
    ...
    if dq.get("cut_short_count", 0) > 0:
        out.append(dict(
            level="warn",
            text=(
                f"มีบิลสถานะ \"ตัดจบ\" (ไม่ใช่ \"จบ\" ตามปกติ) <b>{_fmt_num(dq['cut_short_count'])} บิล</b> "
                f"มูลค่ารวม {_fmt_thb(dq['cut_short_amount'])} ควรตรวจสอบความหมายกับฝ่ายที่เกี่ยวข้อง"
            ),
        ))

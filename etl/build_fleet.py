...
ไฟล์ต้นทางเป็น "บันทึกการปล่อยรถรายเที่ยว" ไม่ใช่ทะเบียน 1 แถวต่อคัน
คอลัมน์: สาขา · วันที่ปล่อยรถ (dd/mm/พ.ศ.) · ประเภทรถ · ชนิดรถ · ทะเบียนรถ
สคริปต์นี้ยุบเหลือ 1 ระเบียนต่อทะเบียน แล้วเขียนเป็น JSON ให้แอปใช้เป็นฐานกลาง
(ไฟล์ 13 ก.ย. 2569: 81,619 แถว 274 คัน · ไฟล์ 24 ก.ย. 2569: 88,085 แถว รวมรถร่วมนอกพิเศษด้วย)
...
กติกา (ตกลงกับเจ้าของข้อมูล 13 ก.ย. 2569):
  ...
  ประเภทรถ           เก็บทั้งสามแบบ รถบริษัท · รถร่วม · รถร่วมนอกพิเศษ (เจ้าของงานให้ทะเบียนมีทุกคัน 24 ก.ย. 2569)
                     แอปคิดต้นทุนรถร่วมนอกพิเศษแบบรถร่วมเอง (costFleetType ใน lib/refdata)
  ช่อง "-"            ถือเป็นช่องว่าง — แถวที่ประเภทหรือชนิดรถเป็น "-" ไม่นับ (ไฟล์ 24 ก.ย. มี 80 แถว)
...
KIND_MAP = {
    ...
    # ไฟล์ 24 ก.ย. 2569 — เจ้าของงานเลือกชื่อในหน้าตั้งค่าให้ (ตรงกับ VEHICLE_ALIASES ใน app/src/lib/refdata/index.ts)
    # "รถ 10 ล้อพ่วง(แม่)" เพิ่มเป็นชนิดใหม่ใน vehicles.json จึงไม่ต้องแมป
    "รถ 10 ล้อพ่วง(ลูก)": "หางพ่วงคอก",
    "รถปิกอัพ": "รถปิกอัพ 3 ตัน",
}

# ค่าที่แปลว่า "ไม่มีข้อมูล" ในไฟล์
BLANKS = {"", "-", "nan"}
...
def build(src: Path) -> None:
    ...
    for c in need:
        df[c] = df[c].map(lambda v: str(v).strip() if pd.notna(v) and str(v).strip().lower() not in BLANKS else None)
    ...
    dash_rows = int((df["ประเภทรถ"].isna() | df["ชนิดรถ"].isna()).sum())
    df = df[df["d"].notna() & df["ทะเบียนรถ"].notna() & df["ประเภทรถ"].notna() & df["ชนิดรถ"].notna()].copy()
    ...
    print(f"  แถวที่ประเภท/ชนิดรถว่างหรือเป็น \"-\" (ไม่นับ): {dash_rows:,} แถว")
    print(f"  ตามประเภทรถหลัก: {dict(Counter(r['fleetType'] for r in out))}")
